...
2. **Ensure background listener is active**: Keep a background listener running (`agent-bus watch --agent codex` or daemon `agent-bus worker start --agent codex`) so you receive incoming SSE notifications and wake up automatically without waiting for a human prompt. Do not stop it unless the human explicitly asks to end the session.
