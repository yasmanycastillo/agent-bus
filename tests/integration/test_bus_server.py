...
async def test_bearer_session_required_instead_of_self_signed_headers(client: AsyncClient, bus_app, tmp_path, monkeypatch):
    ...
    # A self-generated key no longer enrolls or authenticates the caller.
    assert (await client.post("/tasks", json={"task_id": "T99", "title": "Signed task"}, headers=headers)).status_code == 401
    assert (await client.post("/tasks", json={"task_id": "T100", "title": "Unsigned task"})).status_code == 401
    session = await bus_app.sessions.create("claude")
    valid = {"Authorization": f"Bearer {session['token']}"}
    assert (await client.post("/tasks", json={"task_id": "T101", "title": "Session task"}, headers=valid)).status_code == 200
    assert (await client.post("/tasks", json={"task_id": "T102", "title": "Bad task"}, headers={"Authorization": "Bearer bad-token"})).status_code == 401
